# VoyageSmart - Il Compagno di Viaggio Definitivo 🌍✈️

**VoyageSmart** è l'applicazione web moderna pensata per pianificare, gestire e condividere i tuoi viaggi con l'intelligenza artificiale al tuo fianco. Centralizza tutto in un'unica piattaforma intuitiva e collaborativa.

![VoyageSmart Hero](./src/assets/hero-travel.jpg)

## 🚀 Funzionalità AI (Novità!)

### 🤖 AI Travel Assistant
*   **Contextual Agent:** Un assistente che conosce ogni dettaglio del tuo viaggio (itinerario, hotel, budget).
*   **Action Mode:** Aggiungi spese o attività all'itinerario semplicemente chattando.
*   **Rich UI:** Ricevi risposte visive con card interattive cliccabili.
*   **Voice Mode:** Parla con l'assistente e ascolta le sue risposte grazie all'integrazione vocale.
*   **Persistent Memory:** La chat ricorda tutto, anche dopo aver ricaricato la pagina.

### ✨ Smart Travel Intelligence (Novità!)
*   **Smart Suggestions:** Una dashboard intelligente che analizza il tuo viaggio e ti avvisa se mancano alloggi, se stai superando il budget o se hai buchi nell'itinerario.
*   **Trip UX Manager:** Navigazione fluida e assistita per accedere a spese, itinerario e alloggi anche quando non hai ancora selezionato un viaggio.

## 🗺️ Pianificazione & Organizzazione

*   **Collaborazione & Chat di Gruppo (Novità!):**
    *   **Chat Real-time:** Comunica istantaneamente con i tuoi compagni di viaggio.
    *   **Reazioni Emoji:** Reagisci ai messaggi con ❤️, 👍, 😂 e altro per un feedback rapido.
    *   **Risposte Dirette (Quote):** Rispondi a messaggi specifici per mantenere l'ordine nelle discussioni.
    *   **Messaggi in Evidenza:** Fissa informazioni cruciali (WiFi, indirizzi) in alto nella chat.
    *   **Sistemi di Sondaggi Avanzati:** Crea sondaggi a scelta singola o multipla per decidere insieme destinazioni o ristoranti.
    *   **Chat-to-Plan ✨:** Trasforma qualsiasi messaggio della chat in un'attività dell'itinerario o in una spesa con un semplice click.
*   **Strumenti di Viaggio & Utility:**
    *   **Convertitore Valuta:** Tassi di cambio in tempo reale.
    *   **Traduttore AI:** Traduzioni naturali bidirezionali.
    *   **Convertitore Unità:** Lunghezza, peso e temperatura.
*   **Documenti di Viaggio (PRO):**
    *   **Travel Wallet:** Accesso rapido ai documenti essenziali (biglietti, voucher, passaporti).
    *   **Cassaforte Cifrata:** Archivia documenti sensibili con cifratura client-side e passphrase.
*   **Mappa Itinerario Interattiva:** Visualizza il tuo percorso su mappa dinamica con calcolo tempi di percorrenza.
*   **Smart Finance:** Tracciamento spese multi-valuta, conversione automatica e gestione debiti.
*   **Gamification & Social (Novità!):**
    *   **Digital Passport:** Statistiche avanzate su Km percorsi, paesi visitati e viaggi completati.
    *   **Badge Dinamici:** Sblocca obiettivi come "Eroe dei Chilometri" o "Globetrotter".
    *   **Distanze Intelligenti:** Calcolo automatico della distanza percorsa tra le tappe del viaggio e dalla tua città di origine.
*   **Collaborazione Real-time:** Invita amici e pianifica insieme con sincronizzazione istantanea.
*   **Export Intelligente:** Scarica il tuo viaggio in formato PDF professionale o esporta il calendario (.ics).

### 💎 Piani & Monetizzazione
*   **Piano Free:** Accesso completo alla pianificazione, AI limitata (5 msg), funzionalità base.
*   **Piano Pro:** AI illimitata, Export PDF/Calendario sbloccati, Travel Wallet + Cassaforte cifrata, Supporto prioritario.

## 🛠️ Tecnologie Utilizzate

*   **Frontend:** React + TypeScript + Vite
*   **AI Engine:** Google Gemini 2.5 Flash
*   **Styling:** Tailwind CSS + Shadcn/ui + Framer Motion
*   **Backend:** Supabase (PostgreSQL, Realtime, Auth, Storage)
*   **Mappe:** Mapbox GL JS

## 📦 Setup Rapido

1. **Installa dipendenze:** `npm install`
2. **Configura .env:** Aggiungi le chiavi per Supabase, Mapbox e Gemini.
3. **Avvia:** `npm run dev`

---
Progetto distribuito sotto licenza MIT. 
