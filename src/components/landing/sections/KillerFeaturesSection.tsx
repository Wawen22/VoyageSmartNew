        ...
        <div className="grid gap-8 lg:grid-cols-3">
          {features.map((feature, index) => (
            <motion.div
              ...
              className={`relative overflow-hidden rounded-3xl border border-border/40 bg-gradient-to-br ${feature.gradient} p-6 shadow-card`}
            >
              <div className="absolute -right-10 -top-10 opacity-[0.08]">
                <feature.icon className="h-32 w-32" />
              </div>
              <div className="relative z-10">
                <div className={`flex h-11 w-11 items-center justify-center rounded-2xl bg-card/70 ${feature.accent}`}>
                  <feature.icon className="h-5 w-5" />
                </div>
                <h3 className="mt-4 text-xl font-bold text-foreground">{feature.title}</h3>
                <p className="mt-2 text-sm text-muted-foreground">{feature.description}</p>

                <div className="mt-5 rounded-2xl border border-dashed border-border/50 bg-card/40 p-4 text-xs text-muted-foreground">
                  {feature.placeholder}
                </div>
